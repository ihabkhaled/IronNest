...
## Simplicity, ownership, agent readiness & delivery checkpoints (43–51)
...
51. **Publish green checkpoints promptly.** Stage one coherent concern and its proof, inspect the staged diff, run its focused deterministic gate, commit conventionally, and push immediately to the explicitly authorized branch. Every pushed commit stands alone: no known-red state, no dependency on later uncommitted files, no hook bypass, and no final multi-concern mega-commit. (rules/31)
...
- [ ] One coherent independently green batch staged, inspected, committed, and pushed promptly to the authorized branch (rule 51)
